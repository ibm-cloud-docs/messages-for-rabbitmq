...
**Storage** - Storage shows the amount of disk space that is allocated to your service. Each member gets an equal share of the allocated space. Your data is replicated across three data members in the RabbitMQ cluster, so the total amount of storage you use is approximately three times the size of your data set.

The minimum storage of an RabbitMQ deployment is 3072 MB, adjustable in step sizes of 3072 MB. This equates to an initial size of 1024 MB per member with 1024 MB increments available.
...
{{site.data.keyword.messages-for-rabbitmq}} is provisioned with a default admin user. You can set or change the the admin password of your deployment by using _Update Password_.
